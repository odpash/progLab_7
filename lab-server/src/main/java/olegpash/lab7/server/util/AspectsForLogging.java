...
public class AspectsForLogging {
    ...
    @After("execution(* olegpash.lab7.server.util.CommandManager.executeServerCommand(..))")
    public void executeServerCommandAdvise(JoinPoint joinPoint) {
        LOGGER.info("Executing server command: " + joinPoint.getArgs()[0]);
    }

    @After("execution(* olegpash.lab7.server.util.CommandManager.executeClientCommand(..))")
    ...
    @After("execution(* olegpash.lab7.server.db.DBManager.*(..))")
